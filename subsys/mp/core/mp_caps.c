...
/*
 * Static pool of mp_caps objects. No heap is used; capacity is bounded by
 * Kconfig at build time.
 */
K_MEM_SLAB_DEFINE_STATIC(mp_caps_slab, sizeof(struct mp_caps), CONFIG_MP_CAPS_POOL_SIZE,
			 __alignof__(struct mp_caps));

static void mp_caps_destroy(struct mp_object *obj)
{
	struct mp_caps *caps = MP_CAPS(obj);
	struct mp_structure *structure, *tmp;

	__ASSERT_NO_MSG(obj != NULL);
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&caps->caps_structures, structure, tmp, node) {
		mp_structure_destroy(structure);
	}
	sys_slist_init(&caps->caps_structures);
	k_mem_slab_free(&mp_caps_slab, caps);
}
...
struct mp_caps *mp_caps_intersect(struct mp_caps *caps1, struct mp_caps *caps2)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&caps1->caps_structures, s1, node) {
		SYS_SLIST_FOR_EACH_CONTAINER(&caps2->caps_structures, s2, node) {
...
{
	struct mp_structure *structure, *dup_structure;
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&caps->caps_structures, structure, node) {
		dup_structure = mp_structure_duplicate(structure);
		if (dup_structure) {
			mp_caps_append(caps_copy, dup_structure);
		}
	}
...
struct mp_caps *mp_caps_fixate(struct mp_caps *caps)
{
	...
	struct mp_structure *fixated_structure, *first;
	...
	first = CONTAINER_OF(node, struct mp_structure, node);
	fixated_structure = mp_structure_fixate(first);
