 ...
 * CONFIG_SAMPLE_WINDOW_NS window, then loops once per second calling
 * clock_monitor_measure() and logging the result in Hz. Which physical
 * clock is measured depends on the overlay's alias target and the
 * binding's `clocks` phandle.
 */
