 ...
 *
 * The capability structures are @ref mp_structure instances chained directly
 * through their own intrusive node, so no per-structure wrapper is allocated.
 */
struct mp_caps {
	struct mp_object object;     /**< Base object */
	sys_slist_t caps_structures; /**< List of @ref mp_structure capabilities */
};
