...
struct nxp_cmu_fc_data {
	/* Serializes the public API */
	struct k_mutex op_mutex;
	/* Protects fields shared with the ISR (latched_events, cfg fields the
	 * ISR reads).
	 */
...
static int nxp_cmu_fc_start(const struct device *dev)
{
	...
	if (data->state != NXP_CMU_FC_STATE_CONFIGURED ||
	    data->cfg.mode != CLOCK_MONITOR_MODE_WINDOW) {
		k_spin_unlock(&data->lock, key);
		k_mutex_unlock(&data->op_mutex);
		return -EINVAL;
	}
...
static int nxp_cmu_fc_stop(const struct device *dev)
{
	...
	key = k_spin_lock(&data->lock);
	running = (data->state == NXP_CMU_FC_STATE_RUNNING);
	k_spin_unlock(&data->lock, key);
