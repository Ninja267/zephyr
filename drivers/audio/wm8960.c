...
#define WM8960_DEFAULT_MCLK_FREQ  12288000U
				      ...
				      struct audio_codec_cfg *cfg)
{
	...
	if ((ratio == 8U || ratio == 11U || ratio == 12U) && (remainder == 0U)) {
		...
		wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_SYSCLKDIV,
...
static int wm8960_audio_fmt_config(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	wm8960_reg_update(dev, WM8960_CLOCK2,
	...
	/* set adc div */
	wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_ADCDIV,
			  val << WM8960_ADCDIV_SHIFT);

	/* set dac div */
	wm8960_reg_update(dev, WM8960_CLOCK1, WM8960_CLOCK1_DACDIV,
...
static int wm8960_set_input_volume(const struct device *dev, uint8_t l, uint8_t r, bool store)
{
	...
	l = CLAMP(l, 0U, WM8960_MAX_VOLUME);
	r = CLAMP(r, 0U, WM8960_MAX_VOLUME);
			      ...
			      struct audio_codec_cfg *cfg)
{
	...
	if (config->clock_source != 0) {
		data->mclk_in_freq = WM8960_DEFAULT_MCLK_FREQ;
		return 0;
	}
				 ...
				 struct audio_codec_cfg *cfg)
{
	...
	if (data->use_pll) {
		...
		wm8960_reg_update(dev, WM8960_POWER2,
...
static int wm8960_set_default_volumes(const struct device *dev)
{
	...
	if (data->route == AUDIO_ROUTE_BYPASS ||
		...
		ret = wm8960_set_input_volume(dev, WM8960_DEFAULT_VOLUME,
					     WM8960_DEFAULT_VOLUME,
					     true);
		if (ret < 0) {
	...
	if (data->route == AUDIO_ROUTE_BYPASS ||
	    ...
	    data->route == AUDIO_ROUTE_PLAYBACK_CAPTURE) {
		ret = wm8960_set_output_volume(dev, WM8960_DEFAULT_VOLUME,
					      WM8960_DEFAULT_VOLUME,
					      true);
		if (ret < 0) {
...
static void wm8960_configure_jack_detect(const struct device *dev)
{
	const struct wm8960_config *config = dev->config;

	if (config->gpio1_sel == WM8960_TEMP_OK) {
		wm8960_reg_update(dev, WM8960_ADDCTL4,
			    ...
			    struct audio_codec_cfg *cfg)
{
	...
	wm8960_configure_jack_detect(dev);
...
static int wm8960_init(const struct device *dev)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(wm8960_reg); i++) {
