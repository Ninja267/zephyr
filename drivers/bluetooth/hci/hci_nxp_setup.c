...
#define IR_TRIGGER_DELAY_MS                             200U
...
int bt_nxp_trigger_ir(const struct device *dev)
{
	...
	/*
	 * Shutdown BT stack and HCI transport before controller IR trigger
	 * and wait for the stack to de-initialize before proceeding with
	 * IR sequence
	 */
	bt_disable();
	k_sleep(K_MSEC(IR_TRIGGER_DELAY_MS));
	...
	LOG_DBG("Downloading FW after IR.");
	...
	LOG_DBG("IR trigger successful, re-enabling BT stack.");
	err = bt_enable(NULL);
	if (err) {
		LOG_ERR("bt_enable failed after IR (err %d)", err);
		fw_upload.is_ir_request = false;
		return err;
	}

	LOG_DBG("IR Successful, BT stack re-enabled.");
	return err;
}
#else
int bt_nxp_trigger_ir(const struct device *dev)
...
int bt_hci_transport_setup(const struct device *dev)
{
	...
	if (!fw_upload.is_setup_done) {
		...
		/* If IR request is initiated, we do clear the is_ir_request flag
		 * irrespective of ret status to ensure application can re-initiate
		 * the request if needed
		 */
		if (fw_upload.is_ir_request) {
			fw_upload.is_ir_request = false;
		}
	}
