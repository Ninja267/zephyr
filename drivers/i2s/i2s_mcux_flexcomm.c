...
struct i2s_mcux_data {
	...
	struct dma_block_config rx_dma_blocks[NUM_DMA_BLOCKS];
				       ...
				       struct i2s_mcux_q_entry *queue_entry)
{
	...
	if (dir == I2S_DIR_RX) {
		...
		blk_cfg->dest_address = (uint32_t)queue_entry->mem_block;
		...
		blk_cfg->source_address = (uint32_t)&base->FIFORD;
		blk_cfg->source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
		blk_cfg->dest_address = (uint32_t)((queue_entry + 1)->mem_block);
...
{
	int ret = 0;
	struct i2s_mcux_q_entry queue_entry[NUM_DMA_BLOCKS];
	const struct i2s_mcux_config *cfg = dev->config;
	struct i2s_mcux_data *dev_data = dev->data;
	struct stream *stream = &dev_data->rx;
	I2S_Type *base = cfg->base;
