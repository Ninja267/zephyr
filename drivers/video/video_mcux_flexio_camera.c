...
struct nxp_flexio_camera_data {
	...
	bool stream_starved;
	bool streaming;
	/* Buffer currently being filled by DMA */
	struct video_buffer *active_buf;
	/* Current line index within active_buf (0..fmt.height-1) */
...
static void nxp_flexio_camera_start_line_dma(struct nxp_flexio_camera_data *data)
{
	...
	data->dma_block.dest_address = (uint32_t)data->active_buf->buffer +
					   ...
					   int status)
{
	...
	if (!data->streaming || data->active_buf == NULL) {
		return;
	}
	...
	/* Last line completed — publish the frame */
	vbuf = data->active_buf;
				     ...
				     struct video_buffer *vbuf)
{
	...
	if (data->stream_starved) {
		/* A VSYNC edge will now be able to pick up a buffer */
		data->stream_starved = false;
	}
...
#define MCUX_FLEXIO_CAMERA_INIT(inst)								\
	...
	static const struct nxp_flexio_camera_config flexio_cam_config_##inst = {		\
		...
		.dma_channel    = DT_INST_DMAS_CELL_BY_NAME(inst, rx, mux),			\
		.dma_slot	= DT_INST_DMAS_CELL_BY_NAME(inst, rx, source),			\
		...
		.shifter_count  = DT_INST_PROP_OR(inst, shifter_count, 8),			\
