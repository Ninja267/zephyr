...
static int mcux_jpegdec_enqueue(const struct device *dev, struct video_buffer *vbuf)//todo
{
	int ret = 0;
	struct mcux_jpegdec_data *data = dev->data;
	 ...
	 * So the output format and capability can only be got after the first frame of JPEG is
	 * fed to the decoder, and it is assumed  that all the following JPEG frames in the
	 * stream have the same format as the first one, so the parsing is done only once.
	 */
...
static int mcux_jpegdec_get_caps(const struct device *dev, struct video_caps *caps)
{
	...
	if (caps->type == VIDEO_BUF_TYPE_OUTPUT) {
		...
		caps->buf_align = MCUX_JPEGDEC_BUF_ALIGN;
	}
				...
				enum video_buf_type direction, uint16_t ind)
{
	...
	res_cap->width_min   = MCUX_JPEGDEC_WIDTH_HEIGHT_MIN;
	res_cap->width_max   = MCUX_JPEGDEC_WIDTH_HEIGHT_MAX;
	res_cap->width_step  = MCUX_JPEGDEC_WIDTH_HEIGHT_STEP;
	res_cap->height_min  = MCUX_JPEGDEC_WIDTH_HEIGHT_MIN;
	res_cap->height_max  = MCUX_JPEGDEC_WIDTH_HEIGHT_MAX;
	res_cap->height_step = MCUX_JPEGDEC_WIDTH_HEIGHT_STEP;
